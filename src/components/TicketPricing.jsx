...
const InfoChip = ({ icon: Icon, text }) => (
  <span className="backdrop-blur-md bg-white/10 border-2 border-white/30 text-white rounded-md inline-flex items-center gap-1 px-2 py-1 text-xs font-semibold shadow-[3px_3px_0_#000]">
...
const Ticket = ({ accent = 'bg-lime-300', stub = 'bg-rose-300', tier, price }) => (
  <div className="relative w-[290px] sm:w-[320px] max-w-md select-none">
    {/* Premium glow behind the ticket */}
    <div className="pointer-events-none absolute -inset-6 rounded-[18px] opacity-70 blur-2xl" aria-hidden>
      <div className="h-full w-full rounded-[18px] bg-[radial-gradient(ellipse_at_center,rgba(56,189,248,0.35)_0%,rgba(168,85,247,0.35)_45%,transparent_70%)]" />
    </div>

    <div className="relative rounded-xl border-4 border-white shadow-[6px_6px_0_#000] overflow-hidden bg-white">
      {/* Sheen */}
      <div className="pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
        <div className="absolute -inset-20 rotate-12 bg-gradient-to-tr from-white/0 via-white/40 to-white/0" />
      </div>
      {/* Main body */}
      <div className={`${accent} p-5 text-black`}> 
      ...
      {/* Stub */}
      <div className={`${stub} p-4 flex items-center justify-between text-black`}> 
...
export default function TicketPricing() {
  ...
  // Keep the curvature on hover; just push further along the arc
  const leftVariants = {
    rest: { x: -120, rotateZ: -12, rotateY: 18, zIndex: 10, transition: { type: 'spring', stiffness: 120, damping: 14 } },
    hover: { x: -180, rotateZ: -12, rotateY: 18, zIndex: 30, transition: { type: 'spring', stiffness: 120, damping: 12 } },
  };
  const centerVariants = {
    rest: { x: 0, rotateZ: 0, rotateY: 0, scale: 1, zIndex: 20, transition: { type: 'spring', stiffness: 120, damping: 14 } },
    hover: { x: 0, rotateZ: 0, rotateY: 0, scale: 1.03, zIndex: 25, transition: { type: 'spring', stiffness: 120, damping: 12 } },
  };
  const rightVariants = {
    rest: { x: 120, rotateZ: 12, rotateY: -18, zIndex: 10, transition: { type: 'spring', stiffness: 120, damping: 14 } },
    hover: { x: 180, rotateZ: 12, rotateY: -18, zIndex: 30, transition: { type: 'spring', stiffness: 120, damping: 12 } },
  ...
  return (
    <section id="tickets" className="container mx-auto px-6 py-16">
      ...
      <p className="text-white/70 max-w-2xl">
        Curved like a hand-held fan. Hover to glide further along the arc while keeping the curvature intact.
      </p>
      ...
      <motion.div
        ...
        animate="rest"
        whileHover="hover"
        className="group relative mt-10 flex h-[520px] items-center justify-center overflow-visible"
        style={{ perspective: 1100 }}
      >
        {/* Ambient glow rail */}
        <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 mx-auto h-44 max-w-4xl rounded-full bg-[radial-gradient(ellipse_at_center,rgba(99,102,241,0.15)_0%,rgba(236,72,153,0.12)_40%,transparent_70%)] blur-3xl" />
